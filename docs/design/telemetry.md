...
When PostHog Logs is explicitly enabled, the same client adds an OpenTelemetry batch processor behind the process logger. Application logs omit client source addresses; connection and registration limiters retain them only in memory. The remote handler additionally filters source addresses, handles, Credential fingerprints, Save data, terminal content, and opponent identity before enqueueing. Its queue is bounded, exports time out, setup failure disables only remote logs, and shutdown shares the telemetry deadline.
